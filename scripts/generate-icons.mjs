/**
 * src/app/icon.svg 에서 PWA용 PNG 아이콘을 만든다.
 *
...
for (const size of [192, 512]) {
  await sharp(svg, { density: 400 }).resize(size, size).png().toFile(`src/assets/icon-${size}.png`);
}

// iOS 홈화면 아이콘
await sharp(svg, { density: 400 }).resize(180, 180).png().toFile('src/assets/icon-180.png');
...
await sharp({ create: { width: 512, height: 512, channels: 4, background: '#6F9A57' } })
  ...
  .png()
  .toFile('src/assets/icon-maskable-512.png');
