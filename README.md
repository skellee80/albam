...
- **입금이 1시간 안에 안 되면 주문이 자동으로 취소되고 재고가 돌아온다.**
  ...
  >
  > ⚠️ 1시간은 짧다. 밤에 주문하고 아침에 입금하려던 손님은 취소된다.
  > 취소 주문이 늘거나 "입금했는데 취소됐다"는 전화가 오면 이 값을 늘리는 것을 생각해 보라.
...
#### 방법 1. 준비된 그림에서 고르기 — 아버지가 할 수 있다

사진 칸 옆에 작은 그림 세 개가 있다. 누르면 그 그림으로 바뀐다. 끝.

지금 들어 있는 건 임시로 그린 밤 그림이다. 진짜 사진으로 바꾸는 건 방법 2다.

#### 방법 2. 진짜 사진으로 바꾸기 — 컴퓨터에서 한 번만 하면 된다

**아버지가 아니라 컴퓨터를 다루는 사람이 하는 일이다.** 한 번 해두면 아버지는
평소처럼 그림 세 개 중에서 고르기만 하면 된다.

1. 밤 사진을 찍어 컴퓨터로 옮긴다. 정사각형으로 자르고 가로 800px 정도로 줄인다.
2. 파일 이름을 **지금 있는 것과 똑같이** 바꾼다 — `daebo.jpg` · `poredan.jpg` · `okgwang.jpg`
3. `public/products/` 폴더에 넣는다.
4. 배포한다.
...
확장자가 `.svg` 에서 `.jpg` 로 바뀌었으므로, 배포 뒤 **재고관리 → 자세히 고치기 → 사진**
칸의 주소도 `/products/daebo.jpg` 로 한 번씩 고쳐 준다.
(`PRESET_IMAGES` 목록도 `src/components/admin/ProductManager.tsx` 에서 같이 바꿔 두면
다음부터는 눌러서 고를 수 있다)
