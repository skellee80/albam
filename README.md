...
```bash
npm run smoke   # 가격 재계산·입금 매칭·자동 취소·재고 복원 등 핵심 로직 132가지 점검
npx tsc --noEmit
npm run build
```
...
두 건을 **합쳐서 한 번에 송금하는 일이 흔하다.** 그러면 입금액이 어느 주문과도 맞지 않아
자동 매칭이 실패한다.

그래서 주문받는 시점에 물어본다 — 입금자명과 연락처가 같은 입금대기 주문이 있으면
**"합쳐서 한 번에 입금" / "따로 입금"** 중에 고르게 한다.

- 합치면 기존 주문에 상품이 더해지고 금액이 하나로 모인다. 주문번호는 그대로다.
- 합칠 때 **입금 기한이 다시 시작된다.** 어제 주문에 오늘 더했는데 몇 시간 뒤
  자동 취소되면 손님이 납득할 수 없기 때문이다.
- 따로 두면 "각 금액을 따로 보내라"고 안내한다.

고르는 화면에서는 **어느 주소로 가는 밤인지 나눠서** 보여준다.

**받는 주소가 달라도 묶을 수 있다.** 주문 하나에는 받는 주소가 하나뿐이라 상품을 합칠 수는
없지만, 그때는 **주문은 따로 두고 입금만 묶는다**(`paymentGroupId`).
배송은 주소별로 한 상자씩 나가고 — 아버지의 포장·송장 작업은 그대로 한 건씩이다 —
입금 매칭만 묶음 합계로 한다. 확정되면 묶인 주문이 **함께** 발송대기로 올라간다.

| 상황 | 처리 |
|---|---|
| 주소가 같다 | 한 주문으로 합친다 (한 상자) |
| 주소가 다르다 | 주문은 따로, 입금만 묶는다 (상자 두 개, 송금 한 번) |
