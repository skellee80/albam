...
```bash
npm run smoke   # 가격 재계산·입금 매칭·자동 정리·재고 복원 등 핵심 로직 158가지 점검
npx tsc --noEmit
npm run build
```
...
| 상품 늘리기 | 재고관리 → 그룹 아래 **+ 〇〇 에 상품 추가** (새 품종이면 **새 그룹 만들기**) |
...
- 손님 주문과 **같은 길로 들어간다** — 가격 재계산도 재고 차감도 똑같다.
...
판매현황에서는 이 둘을 **한 그래프에 쌓아** 보여준다. 초록이 인터넷 주문,
갈색이 직접 넣은 주문이다. 합계만 보면 사이트가 실제로 얼마나 일하고 있는지 알 수 없다.
