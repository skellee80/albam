# 칠갑산 석촌 유기농 햇 밤 — 무통장 입금 전용 밤 판매 사이트

충남 칠갑산 석촌에서 유기농 햇 밤(대보·포르단·옥광)을 파는 1인 판매자용 사이트다.
...
```bash
npm run smoke   # 가격 재계산·입금 매칭·자동 취소·재고 복원 등 핵심 로직 120가지 점검
npx tsc --noEmit
npm run build
```
...
선물로 여러 곳에 보내는 주문이 흔한데, 주문 하나에는 받는 주소가 하나뿐이라
주소가 다른데 합치면 한 곳으로만 나간다. 그래서 주소가 다르면 빨간 경고를 띄우고
합치기 버튼에도 어느 주소로 가는지 적어 둔다.
